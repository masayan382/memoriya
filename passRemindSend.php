    ...
    if(empty($err_msg)){
      ...
      try {
        ...
        if($stmt && array_shift($result)){
          ...
          //メールを送信
          $from = 'info@memoriya.xxx';
          $to = $email;
          $subject = '【パスワード再発行認証】｜Memoriya';
          $comment = <<<EOT
本メールアドレス宛にパスワード再発行のご依頼がありました。
下記のURLにて認証キーをご入力頂くとパスワードが再発行されます。

パスワード再発行認証キー入力ページ：http://localhost/memoriya/login.php
認証キー：{$auth_key}
※認証キーの有効期限は30分となります

認証キーを再発行されたい場合は下記ページより再度再発行をお願い致します。
http://localhost/memoriya/passRemindSend.php

////////////////////////////////////////
Memoriyaカスタマーセンター
URL  http://memoriya.xxx/
E-mail info@memoriya.xxx
////////////////////////////////////////
EOT;
